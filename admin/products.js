...
function getOfferState(product) {
    const isOnOffer = product?.is_on_offer === true;
    const regularPrice = Number(product?.original_price ?? product?.price ?? 0);
    const offerPrice = Number(product?.price ?? 0);
    return { isOnOffer, regularPrice, offerPrice };
}
                ...
                ${product.is_on_offer
                    ? `
                        <div style="display:flex;flex-direction:column;gap:3px;">
                            <strong style="color:var(--accent,#f5c400);">${formatAdminPrice(product.price)}</strong>
                            <small style="text-decoration:line-through;opacity:.65;">${formatAdminPrice(product.original_price)}</small>
                            <span style="font-size:11px;font-weight:700;color:#22c55e;">ON OFFER</span>
                        </div>`
...
function openProductForm(product = null) {
    ...
    const regularPriceField = document.getElementById("newProductRegularPrice");
    const offerPriceField = document.getElementById("newProductOfferPrice");
    const offerToggle = document.getElementById("newProductIsOnOffer");

    const offerState = getOfferState(product);

    if (regularPriceField) regularPriceField.value = product ? offerState.regularPrice : "";
    if (offerPriceField) offerPriceField.value = product?.is_on_offer ? offerState.offerPrice : "";
    if (offerToggle) offerToggle.checked = product?.is_on_offer === true;
...
async function saveProductFromForm() {
    ...
    const regularPrice = Number(document.getElementById("newProductRegularPrice")?.value);
    const offerPriceRaw = document.getElementById("newProductOfferPrice")?.value.trim() ?? "";
    const isOnOffer = document.getElementById("newProductIsOnOffer")?.checked === true;
    const offerPrice = offerPriceRaw === "" ? NaN : Number(offerPriceRaw);

    const price = isOnOffer ? offerPrice : regularPrice;
    ...
    if (isOnOffer) {
        ...
        if (offerPrice >= regularPrice) {
            alert("The offer price must be lower than the regular price.");
            return;
        }
    }
    ...
    const record = {
        ...
        category,
        price,
        original_price: isOnOffer ? regularPrice : null,
