...
function addToCart(productId) {
  // Use the same cart implementation + success popup everywhere when available.
  // This keeps Home/featured cards and Shop cards visually consistent.
  ...
  if (existing) {
    ...
    const resolvedImage = typeof resolveProductImage === "function"
      ? resolveProductImage(product)
      : product.image;

    const productToSave = {
      ...product,
      image: resolvedImage ? getStoreAssetUrl(`images/products/${resolvedImage}`) : null,
      quantity: 1,
    };
...
function performSiteSearch(input) {
  if (!input) return;
  ...
  const shopUrl = getStorePageUrl("shop.html");
  window.location.href = `${shopUrl}?search=${encodeURIComponent(search)}`;
}

// Header search on internal pages
function searchProducts() {
  performSiteSearch(document.getElementById("searchInput"));
}

// Compatibility helpers for older page headers.
function goToSearch() {
  performSiteSearch(document.getElementById("productSearch") || document.getElementById("searchInput"));
}

function searchFromCart() {
  performSiteSearch(document.getElementById("cartSearch"));
}

// Header search on the homepage
function searchHome() {
  performSiteSearch(document.getElementById("homeSearch"));
}

// Make Enter work in every site-wide header search box.
document.addEventListener("keydown", (event) => {
  if (event.key !== "Enter") return;
  const input = event.target;
  if (!(input instanceof HTMLInputElement)) return;
  if (["searchInput", "homeSearch", "productSearch", "cartSearch", "reviewsSearch", "searchButtonInput", "contactSearch"].includes(input.id)) {
    event.preventDefault();
    performSiteSearch(input);
  }
});
