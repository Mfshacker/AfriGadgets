...
function renderCart() {
  ...
  cart.forEach((item) => {
    ...
    const imageHtml = cartImageUrl
      ? `<img src="${cartImageUrl}" alt="${escapeHtml(item.name)}" style="width:100%; height:100%; object-fit:contain;" onerror="this.parentElement.innerHTML='<i class=\'fa-solid ${item.icon || 'fa-mobile-screen'} \'></i>'">`
    ...
    itemElement.innerHTML = `
            ...
            <div class="cart-product-info">
                <span class="product-category">${escapeHtml(item.category || "PRODUCT")}</span>
                <a href="/product?id=${item.id}" class="cart-product-name">${escapeHtml(item.name)}</a>
